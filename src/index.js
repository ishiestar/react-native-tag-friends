...
export class Editor extends React.Component {
  static propTypes = {
    ...
    extractionKey: PropTypes.string,
  ...
  onSuggestionTap = user => {
    ...
    const username = `@${
      this.props.displayKey ? user[this.props.displayKey] : user.username
    }`;
  ...
  formatText(inputText) {
    ...
    const formattedText = [];
    let lastIndex = 0;
    let { displayKey, extractionKey } = this.props;
    if (!displayKey) {
      displayKey = 'username';
    }
    if (!extractionKey) {
  ...
  formatTextWithMentions(inputText) {
    ...
    this.mentionsMap.forEach((men, [start, end]) => {
      ...
      formattedText = formattedText.concat(
        `@[${men[displayKey]}](id:${men[extractionKey]})`,
      );
      if (
        EU.isKeysAreSame(EU.getLastKeyInMap(this.mentionsMap), [start, end])
      ) {
